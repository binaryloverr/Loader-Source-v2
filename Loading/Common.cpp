...
int detections;
std::string ConsoleTitleDetections;

VOID print_detected()
{
	detections += 1;
	ConsoleTitleDetections = "Current Detections: " + std::to_string(detections);
...
VOID execute_protection(int(*callback)(), const TCHAR* szMsg)
{

	SetConsoleTitleA(ConsoleTitleDetections.c_str());
